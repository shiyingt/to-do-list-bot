...
class DBHelper:
    ...
    def setup(self):
      # create table with tablename items
        stmt = "CREATE _____ IF NOT EXISTS items (description ___)"
        self.conn.execute(stmt)
        self.conn.commit()

    def add_item(self, item_text):
        stmt = "_____ INTO items VALUES (?)"
        args = (item_text, )
        self.conn.execute(stmt, args)
        self.conn.commit()

    def delete_item(self, item_text):
        stmt = "DELETE FROM items WHERE _______"
        args = (item_text, )
        self.conn.execute(stmt, args)
        self.conn.commit()

    def get_items(self):
        stmt = "SELECT _____ FROM ____"
    ...
    def check_exists(self, item_text):
        stmt = "_____ description FROM items WHERE description = (?)"
        args = (item_text, )
